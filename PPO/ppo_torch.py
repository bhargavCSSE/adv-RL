...
class ActorNetwork(nn.Module):
    def __init__(self, n_actions, input_dims, alpha, fc1_dims=200, fc2_dims=200, chkpt_dir='tmp/ppo'):
...
class Agent:
    ...
    def choose_action(self, observation, prev_observation=0):
        ...
        for _ in range(self.n_epochs):
            ...
            for t in range(len(reward_arr) - 1):
                ...
                for k in range(t, len(reward_arr)-1):
                    delta = reward_arr[k] + self.gamma*values[k+1]*(1-int(done_arr[k])) - values[k]
                    a_t += discount*delta
